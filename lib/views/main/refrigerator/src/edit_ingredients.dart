...
class EditIngredients extends StatefulWidget {
  ...
  const EditIngredients({super.key, required this.ingredientModel});
...
class _EditIngredientsState extends State<EditIngredients> {
  ...
  void initState() {
    ...
    super.initState();
    fileUrl = widget.ingredientModel.ingredientImage;
    titleController.text = widget.ingredientModel.ingredientName;
    amountController.text = widget.ingredientModel.originalAmount.toString();
    unitController.text = widget.ingredientModel.unit;
    mfgController.text = widget.ingredientModel.mfg.toString();
    expController.text = widget.ingredientModel.exp.toString();
  }
  @override
  void dispose() {
  ...
  void edit(FridgeState state) async{
    ...
    if(titleController.text.isEmpty || amountController.text.isEmpty || mfgController.text.isEmpty || expController.text.isEmpty || unitController.text.isEmpty){
      ...
      IngredientModel ingredientModel = IngredientModel(
        ingredientId: widget.ingredientModel.ingredientId, 
        ingredientName: titleController.text, 
        ingredientImage: file == null && fileUrl.isEmpty ? foodDefaultImage : fileUrl, 
        userId: widget.ingredientModel.userId, 
        originalAmount: double.parse(amountController.text), 
        consumption: widget.ingredientModel.consumption, 
        unit: unitController.text, 
        mfg: DateTime.parse(mfgController.text), 
        exp: DateTime.parse(expController.text)
      );
      state.editIngredientsInFridge(ingredientModel);
      context.loaderOverlay.hide();
      Message.showScaffoldMessage(context, "editIngredientSuccess".tr(), AppColors.green);
